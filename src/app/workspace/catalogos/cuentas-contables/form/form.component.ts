...
export class FormComponent implements OnInit {
	...
	constructor(
		...
		if (this.router.url.includes('/nuevo')) {
			...
			this.activeRoute.params.subscribe((params) => {
				this.cuentasID = Number(params.ID);
			});
			this.edit = true;
			this.baseKey = 'edit';
			this.patchForm();
	...
	async patchForm() {
		this.form.patchValue({
			empresaID: this.cuentas.empresaID,
			padreCuentaID: this.cuentas.padreCuentaID,
			codigo: this.cuentas.codigo,
			nombre: this.cuentas.nombre,
		});
	}
